...
import type { QuoteState, QuoteLineItem, SKU, DealSetup, YearAdjustment } from '@/types/quote';
...
export function useQuote() {
  ...
  const getYearSubtotal = useCallback(
    (year: number): number => {
      ...
      return items.reduce(
        (sum, item) => sum + calculateLineTotal(item, yearAdj, applyAdj, state.deal.dealType),
        0
      );
    },
